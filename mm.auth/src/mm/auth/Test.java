...
public class Test {
  ...
  public static void main(String[] args) {
    ...
    sessionId = target.path("rest").path("session")
        .path("createSession").request().accept(MediaType.TEXT_PLAIN).get(String.class);
    //System.out.println("SessionID: "+ sessionId);
    //System.out.println(target.path("rest").path("hello")
    //.path("getId").request().get(String.class));
    /*try {
      Thread.sleep(61000);
    ...
    System.out.println(target.path("rest").path("session")
        .path(sessionId).request().get(String.class));
  ...
  private static URI getUrlWithSessionId() throws MalformedURLException, IllegalArgumentException, UriBuilderException {
    ...
    return UriBuilder.fromUri("http://localhost:8080/com.vogella.jersey.first/rest/session/" + sessionId).build();    
  }
}