...
public class NetGearGS108Tv2 implements NetComponent {
  ...
  public void createLocalVlans() {
    int min = NetData.getLOCAL_VLAN_RANGE_MIN();
    int max = NetData.getLOCAL_VLAN_RANGE_MAX();

    for (int i = min; i <= max; i++) {
      this.localVlans.add(new VLan(i, false));
    }
  }
   ...
   * 
   * int[] ports = {port}; boolean[] trunks = {trunk};
   * 
   ...
   * 
   * if(response.getStatus() != 200) { return errorHandler("addPorts",
   * "could not get egressPorts/Untagged because of: \n" +
   * Arrays.toString((String[]) response.getEntity())); }
   * 
   * String currentEgress = ((String[]) response.getEntity())[0]; String
   * currentUntagged = ((String[]) response.getEntity())[1]; String newEgress =
   * currentEgress; String newUntagged = currentUntagged;
   * 
   ...
   * 
   * newUntagged = newUntaggedBuilder.toString(); } else { setPVID(ports[i],
   * vlanId); }
   * 
   * response = setEgressPorts(vlanId, newEgress); //TODO fehlerbehandlung
   ...
   * 
   * <p>
   * All entries affiliated with this VLan are deleted and can not be restored.
   * It is not possible to destroy a VLan which is static (Management/Power) or
   * out of the allowed range of the API (see VLAN.xml).
   * 
   * @param vlanId
   ...
   * Examples:
   * 
   * <div style="text-indent:30px;"> <code>AA</code> sets the 1st, 3rd, 5th, 7th
   * port as an egress port, same as <code>10101010</code> </div>
   * 
   ...
   * 
   * <div style="text-indent:30px;"> <code>AA</code> sets the 1st, 3rd, 5th, 7th
   * port as an untagged port, same as <code>10101010</code> </div>
   * 
   * <div style="text-indent:30px;"> <code>00001111</code> sets the last four
   ...
   * { LinkedList<Integer> list = new LinkedList<Integer>(); list.add(port);
   ...
   */
  /**
   * public Response setVLan(LinkedList<Integer> ports, boolean global, int
   * vlanId, String name) {
   * 
   * for (Integer integer : ports) { if (trunks.contains(integer)) { return
   * Response.status(500).entity("Can not set VLan on TrunkPort").build(); } }
   * 
   * if (global) { ports.addAll(trunks); }
   * 
   * String egressPorts = "";
   * 
   * for (int i = 1; i <= 8; i++) { if (ports.contains(i)) { egressPorts =
   * egressPorts + "1"; } else { egressPorts = egressPorts + "0"; } }
   * 
   * egressPorts = String.format("%02x", Integer.parseInt(egressPorts, 2));
   * 
   * Response response = setRowStatus(vlanId, 4); if (response.getStatus() !=
   * 200) { System.out.println("ROW STATUS FAILED"); destroyVlan(vlanId); return
   * response; }
   * 
   * response = setStaticName(vlanId, name); if (response.getStatus() != 200) {
   * destroyVlan(vlanId); return response; }
   * 
   * response = setEgressPorts(vlanId, egressPorts); if (response.getStatus() !=
   * 200) { destroyVlan(vlanId); return response; }
   * 
   * response = setUntaggedPorts(vlanId, UNTAGGED_PORTS); if
   * (response.getStatus() != 200) { destroyVlan(vlanId); return response; } for
   * (Integer port : ports) { if (!trunks.contains(port)) { response =
   * setPVID(port, vlanId); if (response.getStatus() != 200) {
   * destroyVlan(vlanId); return response; } }
   * 
   * } return Response.ok().build(); }
   **/
   ...
   * 
   * HTTP status codes:
   * <li>200: Requested information could be obtained, body has the name of VLan
   * or "noSuchInstance".</li>
   * <li>500: Requested information could not be obtained, boy contains specific
   * information.</li>
   *
   * <p>
   * 
  ...
  /**
   * Returns a Response Object with the location of the error (where input) and
   * the error(error String). Adds some format and method of the object, which
   * caused the error.
   * 
   * @param where
