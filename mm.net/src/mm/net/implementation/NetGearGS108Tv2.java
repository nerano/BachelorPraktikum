...
public class NetGearGS108Tv2 implements NetComponent {
     ...
     * 200) { System.out.println("ROW STATUS FAILED"); destroyVlan(vlanId);
     * return response; }
     * 
     ...
     * (response.getStatus() != 200) { destroyVlan(vlanId); return response; }
     * for (Integer port : ports) { if (!trunks.contains(port)) { response =
