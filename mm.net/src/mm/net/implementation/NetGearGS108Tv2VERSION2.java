...
public class NetGearGS108Tv2VERSION2 {
	 ...
	 * @param add
	 */
	public NetGearGS108Tv2VERSION2(String identifier, String add) {
...
public Response setEgressPorts(int vlanId, String variable) throws IOException {
		...
		Variable var = OctetString.fromHexString(variable);
		PDU pdu = new PDU();
	...
	public Response getStaticName(int vlanId) {
		try {
			...
			if (event != null) {
				//TODO DAS IST KEIN GUTES ERRORHANDLING DA DAS EVENT BEI TIMEOUT NICHT NULL IST
				if(event.getResponse() == null) { System.out.println("HALLO DAS IST NULL"); }
	...
	public Response getEgressPorts(int vlanId) {
		try {
			...
			if (event != null) {
				...
				String output = String.format("%8s", s).replace(' ', '0');
