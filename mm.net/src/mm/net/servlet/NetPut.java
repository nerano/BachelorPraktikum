 ...
 * <li>addPort / setPort</li>
 * 
 * <li>addTrunk / setTrunk</li>
 * 
 * <li>removePort / removeVlan</li>
 *
 * <p>
 * In General:
 *
 * <p>
 * addX adds ports to an already existing VLAN, eventually replaces already
 * existing configurations for members of this VLAN, but does not change any
 * other members. Does not work on non existing VLANs and returns an error.
 *
 * <p>
 * setX sets the provided VLAN anew, if it already existed, and replaces every
 * existing configuration. If the VLAN did not exist it is created.
 * 
 *
 */
@Path("/put")
public class NetPut {
   ...
   *
   * <p>
   * The ports in the VLAN object are added as members of the VLAN. These ports
   * are (U) untagged, every frame leaving this port loses his VLAN ID tag.
   * Every other member of the VLAN is not changed, the submitted ports are only
   * added to the VLAN.
   *
   * <p>
   ...
   * destroy the VLAN, even when all members were removed, the VLAN still exists
   * on the NetComponent. PVID/Native ID of the ports is changed to 1.
   * 
   ...
   * exist anymore after calling this method. Does not remove the VLAN from all
   * NetComponents per se, but only from a NetComponent if it was a member of
   * the submitted VLAN. Sets the PVID of all ports in this VLAN on 1.
   *
   * <p>
   * URI: <code>baseuri:port/mm.net/rest/put/removePort</code>
   * 
